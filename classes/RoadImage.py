...
class RoadImage(np.ndarray):
    ...
    def __new__(cls, input_array=None, *, filename=None, cspace=None, src_cspace=None):
        ...
        if src_cspace != cspace and (src_cspace != 'RGB' or cspace == 'GRAY'):
            ...
            raise ValueError(
                'RoadImage: Cannot autoconvert from ' + str(src_cspace) + ' to ' + str(cspace) + ' in ctor.')
    ...
    def _inherit_attributes(self, obj, binary_test=True):
        ...
        if self.binary and binary_test:
            ...
            maxi = v.max()
            mini = v.min()
            if maxi.dtype == np.float32:
                epsilon = 10. * np.finfo(type(maxi)).eps
            else:
                epsilon = 1
                ...
                if coords1 == [0] * len(coords1) and coords2 == list(obj.shape):
                    ...
                    if self.shape[-3:] != obj.shape[-3:]:
                        if self.ndim == 1:
                            ...
                            op = ((RoadImage.channels,
                                   (range(0, obj.shape[-1], (obj.shape[-1] - 1) // (self.shape[-1] - 1)),)),)
                        else:
    ...
    def find_common_ancestor(cls, lst):
        ...
        for i, img in enumerate(lst):
            if not (issubclass(type(img), RoadImage)):
                raise TypeError(
                    'RoadImage.find_common_ancestor: List elements must be type RoadImage. Check element %d.'
                    % i)
    ...
    def make_collection(cls, lst, size=None, dtype=None, concat=False):
        ...
        for i, img in enumerate(lst):
            if not (issubclass(type(img), RoadImage)):
                raise ValueError(
                    'RoadImage.make_collection: List elements must be type RoadImage. Check element %d.' % i)
        if dtype is None:
            dtype = lst[0].dtype
        ...
        for i, img in enumerate(lst):
            if img.shape[:-3] != shape_coll:
                if not (concat) or img.shape[1:-3] != shape_coll[1:]:
                    raise ValueError(
                        'RoadImage.make_collection: List elements must have the same collection structure. '
                        + 'Check element %d.' % i)
    ...
    def find_child(self, op):
        ...
        if issubclass(type(strop), str):
            for kop in self.child.keys():
                ...
                assert issubclass(type(kop[0]),
                                  tuple), 'RoadImage.find_child: BUG: child.keys() still contains short ops!'
                if len(kop) > 1:
            ...
            if nb_ch == 1:
                if cmap is None:
                    if self.colorspace == 'GRAY':
                        ...
                        cmap = 'cubehelix'
                    elif self.colorspace == 'HLS' or self.colorspace == 'HSV':
                        cmap = 'hsv'
                    else:
                        cmap = 'gnuplot2'
    ...
    def gradients(self, tasks, sobel_kernel=3, minmag=0.04):
        ...
        if not (set(tasks).issubset({'x', 'y', 'absx', 'absy', 'mag', 'angle', 'absangle'})):
            raise ValueError(
                "RoadImage.gradient: Allowed tasks are 'x','y','absx','absy','mag','angle' and 'absangle'.")
        if sobel_kernel % 2 == 0:
        ...
        # Scaling factor
        if sobel_kernel <= 9:
            scale_f = 1. / RoadImage.SOBELMAX[sobel_kernel]
        else:
            # Compute scale_f: convolve with an image with a single 1 in the middle
            ...
            scale_f = 1. / np.sum(kernel[:, sobel_kernel:])
        ...
        for ch in range(RoadImage.image_channels(self)):
            ...
            for img, gray in enumerate(flat_ch):
                ...
                if set(tasks).intersection({'x', 'absx', 'mag', 'angle'}):
                    derx = scale_f * cv2.Sobel(gray, cv2.CV_32F, 1, 0, ksize=sobel_kernel)
                    abs_derx = np.abs(derx)
                if set(tasks).intersection({'y', 'absy', 'mag', 'angle'}):
                    dery = scale_f * cv2.Sobel(gray, cv2.CV_32F, 0, 1, ksize=sobel_kernel)
        ...
        for ix, img in enumerate(self):
            ...
            for iy, line in enumerate(img):
                ...
                for ic in range(self.shape[3]):
                    if invertx:
                        l = np.convolve(line[:, ic], -kernel, mode='same')
                    else:
                        l = np.convolve(line[:, ic], kernel, mode='same')
                    ret[ix, iy, :, ic] = l
            # Y-pass
            for iy in range(self.shape[2]):
                ...
                for ic in range(self.shape[3]):
                    if inverty:
                        l = np.convolve(col[:, ic], -kernel, mode='same')
                    else:
                        l = np.convolve(col[:, ic], kernel, mode='same')
                    ret[ix, :, iy, ic] += l
        ret.binary = False
    ...
    def _retrieve_cal(self, cal=None):
        ...
        if cal is None:
            if my_cal is None:
                raise ValueError(
                    'RoadImage._retrieve_cal: No CameraCalibration associated with this image. Pass arg cal.')
            else:
                return my_cal
        elif my_cal is None:
    ...
    @generic_search()
    @flatten_collection
    def curves(self, key, *, dir='x', width=0.12, origin, scale, wfunc=None, minpts=150, sfunc=None, cal=None,
               **kwargs):
        """
        ...
        for i, j in iters:
            ...
            if wrap_w:
                weight_func = partial(wfunc, val=np.concatenate([val1, val2], axis=0))
            else:
                weight_func = wfunc
            ...
            if dir == 'x':
                ...
                z_max = np.max(Y)
                warnings.warn("RoadImage.curves: z_max implementation for dir='y' does not yet use wfunc.",
                              RuntimeWarning)
            ...
            if np.all(lbounds):
                ...
                bounds_score = (1 + np.argmin(lbounds[index])) / len(lbounds)
            ...
            if sfunc:
                try:
                    ...
                    if cust_score < 0.1:
                        cust_score = 0.1  # do not formally eliminate solutions
                    elif cust_score > 1:
                        cust_score = 1  # cap influence
    ...
    def find_lines(self, cal, *, z=70, h=0, scale=(.02, .1)):
        ...
        def eval(key, warpimage, origin, scale, x0, z0):
            ...
            try:
                ...
                linebuf.track(state)
                if dens is None:
                    state.lines.unset(key, 'dens')
                else:
                    state.lines.set(key, 'dens', dens)
                if z_max is None:
                    state.lines.unset(key, 'zmax')
                else:
                    state.lines.set(key, 'zmax', z_max)
        ...
        def estimates(lines, keepn=6, update=True, bestfirst=True, order=4):
            ...
            for li in lines:
                ...
                try:
                    ...
                    linelib.set(curK(li), 'eval',
                                MIN_EVAL)  # 0.001 is a very low eval. Avoid 0 to make sure total is never 0.
        ...
        _state('lines', self.lines)  # Line instance comes from self RoadImage instance, but is recycled for each frame
        # Tunables
    ...
    def centroids(self, *, x, lanew, scale):
        ...
        for level in range(1, nb_layer):
            ...
            if len(conv_signal[min_index:max_index]) == 0:
                newx = 0
            else:
                newx = np.argmax(conv_signal[min_index:max_index])
            if newx == 0:
